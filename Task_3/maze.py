...
class Maze:
    ...
    def find_path(self):
        ...
        """
        search_stak = Stack()
        search_stak.push(self._start_cell)

        coords = self._start_cell
        self._mark_path(self._start_cell.row, self._start_cell.col)

        while coords != self._exit_cell:

            list_of_tries = [(-1, 0), (0, 1), (1, 0), (0, -1)]

            for i, j in list_of_tries:
                if self._valid_move(coords.row + i, coords.col + j):
                    coords = _CellPosition(coords.row + i, coords.col + j)
                    self._mark_path(coords.row, coords.col)
                    search_stak.push(coords)
                    break
            else:
                self._mark_tried(coords.row, coords.col)
                search_stak.pop()

                try:
                    coords = search_stak.peek()
    ...
    def __str__(self):
        ...
        for row in range(self.num_rows()):
            for col in range(self.num_cols()):
                the_str += '_ ' if self._maze_cells[row, col] == None else self._maze_cells[row, col] + ' '
            the_str += "\n"
